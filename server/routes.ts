...
// Shape of the `details` JSONB payload written by system_metrics log entries
// (used by GET /api/metrics/resources).
interface SystemMetricDetails {
  cpu?: { usage?: number };
...
const errorHandler = (
  ...
  // must remain in the signature even though it is unused here.
...
/**
 * Returns a shallow copy of `value` without its `password` field.
 */
const omitPassword = <T extends { password: string }>(
  value: T,
): Omit<T, "password"> => {
  const result = { ...value } as Omit<T, "password">;
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const response: SuccessResponse<ApiKey[]> = {
          success: true,
          data: results as unknown as ApiKey[],
  ...
  app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const usersWithoutPasswords = results.map((user) => omitPassword(user));
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const userWithoutPassword = omitPassword(userData);
  ...
  app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const userWithoutPassword = omitPassword(result[0]);
  ...
  app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const conditions = [];
        if (level && level !== "all")
          conditions.push(eq(logsTable.level, level));
        if (source && source !== "all")
          conditions.push(eq(logsTable.source, source));
        if (project && project !== "all")
          conditions.push(eq(logsTable.project, project));
        ...
        if (timeRange && timeRange !== "custom") {
          ...
          fromDate.setTime(
            now.getTime() - (rangeMap[timeRange] || rangeMap["24h"]),
          );
          conditions.push(
            sql`${logsTable.timestamp} >= ${fromDate.toISOString()}`,
          );
        } else if (timeRange === "custom" && (from || to)) {
          if (from)
            conditions.push(
              sql`${logsTable.timestamp} >= ${new Date(from).toISOString()}`,
            );
          if (to)
            conditions.push(
              sql`${logsTable.timestamp} <= ${new Date(to).toISOString()}`,
            );
        }

        const validColumns: Record<string, SQLWrapper> = {
  ...
  app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (source && source !== "all")
          conditions.push(eq(logsTable.source, source as string));
        ...
        res.json({
          ...
          data: {
            currentCpuUsage: hasRealData
              ? Number(
                  (latest.details as SystemMetricDetails).cpu?.usage || 0,
                ) * 100
              : 0,
            maxCpuUsage: hasRealData
              ? Math.max(
                  ...results.map(
                    (r) => (r.details as SystemMetricDetails).cpu?.usage || 0,
                  ),
                ) * 100
              : 0,
            avgCpuUsage: hasRealData
              ? (results.reduce(
                  (sum, r) =>
                    sum + ((r.details as SystemMetricDetails).cpu?.usage || 0),
                  0,
                ) /
                  results.length) *
                100
            ...
            currentMemoryUsage: hasRealData
              ? Number(
                  (latest.details as SystemMetricDetails).memory
                    ?.usage_percent || 0,
                )
              : 0,
            maxMemoryUsage: hasRealData
              ? Math.max(
                  ...results.map(
                    (r) =>
                      (r.details as SystemMetricDetails).memory
                        ?.usage_percent || 0,
                  ),
                )
              : 0,
            avgMemoryUsage: hasRealData
              ? results.reduce(
                  (sum, r) =>
                    sum +
                    ((r.details as SystemMetricDetails).memory?.usage_percent ||
                      0),
                  0,
                ) / results.length
              : 0,
            cpuUsageData: results
              .map(
                (r) =>
                  Number((r.details as SystemMetricDetails).cpu?.usage || 0) *
                  100,
              )
              .reverse(),
            memoryUsageData: results
              .map((r) =>
                Number(
                  (r.details as SystemMetricDetails).memory?.usage_percent || 0,
                ),
              )
              .reverse(),
            diskUsageData: [],
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const [recentAlerts] = await db
          ...
          .from(alerts)
          .where(
            sql`created_at >= ${new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()}`,
          );
